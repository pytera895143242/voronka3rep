...
async def answer_push_inline_button(call, state: FSMContext):
    if call.data == 'sprint_online':
        ...
        bat_a = types.InlineKeyboardButton(text='ДАЛЕЕ🔆', callback_data='go_1')
        markup.add(bat_a)
        await bot.copy_message(from_chat_id=content, chat_id=call.message.chat.id, message_id = 163,reply_markup=markup)

    if call.data == 'go_1':
        markup = types.InlineKeyboardMarkup()
        bat_a = types.InlineKeyboardButton(text='ПОДРОБНЕЕ🧐', callback_data='go_2')
        markup.add(bat_a)
    ...
    if call.data == 'go_3':
        ...
        await state.update_data(v1='false')
        markup = types.InlineKeyboardMarkup()
        ...
        try:
            if ((await state.get_data())['v1']) == 'true':
                flag = True
            else:
                flag = False
        except:
    ...
    if call.data == 'go_4':
        ...
        if flag == False:
            ...
            markup.add(bat_a)
            await bot.copy_message(from_chat_id=content, chat_id=call.message.chat.id, message_id=182,reply_markup=markup)
    ...
    if call.data == 'go_12':
        ...
        if flag == False:
            ...
            await bot.copy_message(from_chat_id=content, chat_id=call.message.chat.id, message_id=196, reply_markup=markup)
            await asyncio.sleep(1800)  # 30 минут
            try:
...
Мой телеграм 👉 @{user}""")
        ...
        await bot.send_message(chat_id=call.message.chat.id, text=f"""Чача раз Чача два 💃🏼💃🏼
...
👉 @{user}""")
        await asyncio.sleep(86400)  # 86 400
        await bot.copy_message(from_chat_id=content, chat_id=call.message.chat.id, message_id=226, caption=f"""⛔️МЕСТА по скидке ЗАКОНЧИЛИСЬ⛔️

Но если ты готов 
Тогда можем лично для тебя
Оставить скидку👉 @{user}""")
        await asyncio.sleep(86400)  # 86 400
        await bot.copy_message(from_chat_id=content, chat_id=call.message.chat.id, message_id=228)
